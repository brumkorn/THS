 ...
 */
import Sheet from 'main/sheet.js';
...
function initListeners(editor) {
  let select = editor.footerToolbar.querySelector("select"),
    newSheetButton =
      editor.footerToolbar.querySelector(".new-sheet-button"),
    sheetBookmarks =
      editor.footerToolbar.querySelector(".sheet-bookmarks"),
    saveButton = document.getElementsByClassName("menu-button")[1],
    resetButton = document.getElementsByClassName("menu-button")[2],
    deleteSheetButton = document.getElementsByClassName("menu-button")[3];
...
Are you shure you want to delete ${editor._currentSheet.name}?
`
    );
    if (!decision) return;
    let delSheetID = editor._currentSheet.ID;
    editor.switchSheet(editor._currentSheet.ID - 1);
    editor.deleteSheet(delSheetID);
  };
  newSheetButton.addEventListener("click", () => editor.addNewSheet());
  select.addEventListener("change", switchSheetHandler);
  sheetBookmarks.addEventListener("click", switchSheetHandler);
  deleteSheetButton.addEventListener("click", deleteSheetHandler);
  resetButton.addEventListener("click", resetDataBasesHandler);
  saveButton.addEventListener("click", () => editor.saveData());
