...
(function () {
    ...
    static parseExpression(inputExp, tbody) {
      ...
      let returnArr = inputArr.map(function (current) {
        ...
        if (matched) {
          ...
          let { rowIndex, colIndex } = Utils.getCellCoordinates(cellName);
  ...
  class SheetEditor {
    ...
    initTable() {
      ...
      let sheetContainer = windowFrame.appendChild(document.createElement("div")),
          tableWrapper = sheetContainer.appendChild(document.createElement("div"));
      tableWrapper.classList.add("table-wrapper");

      let colHeaderWrapper = sheetContainer.insertBefore(document.createElement("div"), tableWrapper);
      colHeaderWrapper.classList.add("col-header-wrapper");
      colHeaderWrapper.appendChild(document.createElement("div"));
      let colHeader = colHeaderWrapper.appendChild(document.createElement("div"));
      colHeaderWrapper.appendChild(document.createElement("div"));
      colHeader.classList.add("col-header");
      colHeader.appendChild(document.createElement("ol"));

      let rowHeader = sheetContainer.insertBefore(document.createElement("div"), tableWrapper);
    ...
    createSheet(columns = this.columns, rows = this.rows, loading = false) {
      ...
      if (loading) {
        sheetCellsData = editor.loadData() ? editor.loadData()[sheetID].cellsList : editor.serverData[sheetID].cellsList;
      }
    ...
    switchSheet(sheetIndex) {
      let sheetBookmarksList = this.footerToolbar.querySelectorAll(".sheet-bookmarks > div"),
    ...
    initListeners() {
      ...
      let switchSheetHdlr = event => {
        if (event.target.parentElement.className === "sheet-bookmarks" || event.target.tagName === "SELECT") {
...
`);
  ...
  class Sheet {
    ...
    constructor(columns, rows, sheetID, cellsList) {
      ...
      this.corner = this.sheetContainer.querySelector(".col-header-wrapper div:first-child");
    ...
    loadCellsData() {
      ...
      for (let cell in sheet.cellsList) {
        ...
        a(currentCell.value);
        sheet.cellsList[cell] = new Cell(currentCell.rowIndex, currentCell.colIndex, sheet.tbody, currentCell.value, currentCell.computedValue);
    ...
    listenersControl(active = true) {
      let cornerHeight = parseFloat(window.getComputedStyle(this.corner).height),
      ...
      let removeLastFocus = () => {
        this.focusedCell.classList.remove("focused-cell", "focused-input-cell");
      };
      let inputDone = () => {
        if (this.formulaMode === true) return;

        let { rowIndex, colIndex, cellName } = Utils.getCellCoordinates(input.parentNode);
        ...
        if (!this.cellsList[cellName] && input.value !== "") {
          this.cellsList[cellName] = new Cell(rowIndex, colIndex, this.tbody);
        }
      ...
      let formulaModeToggle = (inputDone = false) => {
        ...
        if (this.formulaMode) {
          ...
          this.tableWrapper.addEventListener("click", formulaPickCellHdlr);
        } else {
          ...
          this.tableWrapper.removeEventListener("click", formulaPickCellHdlr);

          let pickedCellsNodes = this.tableWrapper.querySelectorAll(".formula-mode-picked-cell");
      ...
      let pullHeadersHdlr = event => {
        ...
        rowHeader.style.top = cornerHeight - this.tableWrapper.scrollTop + "px";
        colHeader.style.left = cornerWidth - this.tableWrapper.scrollLeft + "px";
      };
      ...
      let headersHighlightHdlr = event => {
        ...
        if (event.target.tagName === "TD") {
          ...
          this.rowHeaderList.children[rowIndex].classList.add("cell-header-highlight");
          this.colHeaderList.children[colIndex].classList.add("cell-header-highlight");
          this.highlightedHeaders.rows.push(this.rowHeaderList.children[rowIndex]);
          this.highlightedHeaders.columns.push(this.colHeaderList.children[colIndex]);
        }
      };
      let invokeInputHdlr = event => {
        if (this.formulaMode === true || event.target === input || event.keyCode === Utils.keyCode.backspace || event.keyCode === Utils.keyCode.del) {
          return;
        }
        ...
        if (event.keyCode && event.keyCode !== Utils.keyCode.enter) {
          input.value = "";
      ...
      let inputKeyDoneHdlr = event => {
        if (event.keyCode === Utils.keyCode.enter) {
          let { rowIndex, colIndex } = Utils.getCellCoordinates(input.parentElement);
      ...
      let formulaPickCellHdlr = event => {
        ...
        if (input.value.search(Utils.regExp.pickingInputEnding) >= 0) {
          let linkEndingPos = input.value.search(Utils.regExp.cellLinkEnding);
          let { cellName } = Utils.getCellCoordinates(event);
          if (linkEndingPos >= 0) {
            let lastPickName = input.value.slice(linkEndingPos);
            let { rowIndex, colIndex } = Utils.getCellCoordinates(lastPickName);
            let lastCellNode = Utils.findCellOnSheet(rowIndex, colIndex, this.tbody);
      ...
      let forcedFormulaModeFocusHdlr = event => {
        if (event.relatedTarget === input || event.relatedTarget === this.inputConsole) return;
      ...
      let clearCellDataHdlr = event => {
        ...
        if (event.keyCode === Utils.keyCode.alt || event.keyCode === Utils.keyCode.backspace) {
          event.preventDefault();
        }
        if (event.target.innerHTML === "") return;
        if (event.keyCode === Utils.keyCode.backspace || event.keyCode === Utils.keyCode.del) {
          let { cellName } = Utils.getCellCoordinates(event);
          delete this.cellsList[cellName];
